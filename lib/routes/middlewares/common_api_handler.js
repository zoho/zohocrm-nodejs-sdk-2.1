...
class CommonAPIHandler {
  ...
  setParam(param) {
    ...
    if (
        this.param.getParameterMap() != null &&
        this.param.getParameterMap().size > 0
    ) {
  ...
  setHeader(header) {
    ...
    if (
        this.header.getHeaderMap() != null &&
        this.header.getHeaderMap().size > 0
    ) {
  ...
  async apiCall(className, encodeType) {
    ...
    try {
      await this.setAPIUrl(connector).catch((err) => {
        throw err;
      });
    } catch (error) {
    ...
    if (
        ...
        Constants.IS_GENERATE_REQUEST_BODY.includes(this.httpMethod.toUpperCase())
    ) {
      ...
      try {
        converterInstance = this.getConverterClassInstance(
            this.contentType.toLowerCase()
        );

        var className = converterInstance.getFileName(
            this.request.constructor.name
        );

        baseName.push(className);

        requestObject = await converterInstance
            .formRequest(this.request, baseName.join("/"), null, null)
            .catch((err) => {
              throw err;
            });
      } catch (error) {
    ...
    try {
      ...
      let headerMap = await this.getHeaders(response.headers).catch(err=>{throw err;});
      ...
      if (
        ...
        let contentTypeHeader =
            response.headers[Constants.CONTENT_TYPE_HEADER.toLowerCase()];
        ...
        returnObject = await converterInstance
    ...
    if (this.apiPath.toString().includes(Constants.HTTP)) {
      if (this.apiPath.toString().includes(Constants.CONTENT_API_URL)) {
        apiPath = apiPath.concat(
            initializer.getEnvironment().getFileUploadUrl()
        );
