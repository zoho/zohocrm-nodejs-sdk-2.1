...
class APIHTTPConnector {
  ...
  async fireRequest(converterInstance) {
    ...
    if (Array.from(Object.keys(this.requestBody)).length > 0) {
      modifiedRequestBody = await converterInstance
          .appendToRequest(this, null);
    }
    ...
    if (initializer.getRequestProxy() != null) {
      ...
      if (requestProxy.getUser() != null) {
        proxyAuthorization =
            requestProxy.getUser() + ":" + requestProxy.getPassword();
      }
    ...
    return got(this.url, requestDetails);
  }
  ...
  toString() {
    ...
    let paramMapString = JSON.stringify(this.parameters, (key, value) =>
        value instanceof Map ? [...value] : value
    );

    return Constants.REQUEST_METHOD.concat(" = ")
        .concat(this.requestMethod)
        .concat(" , ")
        .concat(Constants.URL)
        .concat(" = ")
        .concat(this.url)
        .concat(" , ")
        .concat(Constants.HEADERS)
        .concat(" = ")
        .concat(headerMapString)
        .concat(" , ")
        .concat(Constants.PARAMS)
        .concat(" = ")
        .concat(paramMapString)
        .concat(".");
  }

  proxyLog(requestProxy) {
    let proxyDetails = Constants.PROXY_SETTINGS.concat(Constants.PROXY_HOST)
    ...
    if (requestProxy.getUser() != null) {
      proxyDetails = proxyDetails
          .concat(" , ")
          .concat(Constants.PROXY_USER)
          .concat(requestProxy.getUser());
    }
