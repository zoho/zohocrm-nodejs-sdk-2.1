...
class OAuthToken extends Token {
  ...
  async authenticate(urlConnection) {
    try {
      ...
      var initializer = await Initializer.getInitializer().catch((err) => {
        throw err;
      });
      ...
      if (oauthToken === null) {
        ...
        token =
            this.refreshToken != null
                ? (
                    await this.refreshAccessToken(user, store).catch((err) => {
                      throw err;
                    })
                ).getAccessToken()
                : (
                    await this.generateAccessToken(user, store).catch((err) => {
                      throw err;
                    })
                ).getAccessToken();
      } else if (
          oauthToken.getExpiresIn() != null &&
          parseInt(oauthToken.getExpiresIn()) - parseInt(new Date().getTime()) <
          5000
      ) {
  ...
  async refreshAccessToken(user, store) {
    ...
    try {
      await this.parseResponse(response.body).catch((err) => {
        throw err;
      });

      if (this.id == null) {
        await this.generateId().catch((err) => {
          throw err;
        });
      }

      await store.saveToken(user, this).catch((err) => {
        throw err;
      });
    } catch (error) {
      if (error instanceof SDKException) {
  ...
  async generateId() {
    let email = (await Initializer.getInitializer()).getUser().getName();

    let builder = Constants.NODEJS + email + "_";
    ...
    if (this.refreshToken != null) {
      builder =
          builder + this.refreshToken.substring(this.refreshToken.length - 32);
    }
