...
class Utility {
  ...
  static async fileExistsFlow(
    ...
    if (
      initializer.getSDKConfig().getAutoRefreshFields() == true &&
      !this.newFile &&
      !this.getModifiedModules &&
      ...
      initializer.getSDKConfig().getAutoRefreshFields() == false &&
      this.forceRefresh &&
      !this.getModifiedModules
    ) {
      this.getModifiedModules = true;
    ...
    if (
      ...
      (recordFieldDetailsJson.hasOwnProperty(moduleAPIName.toLowerCase()) &&
        recordFieldDetailsJson[moduleAPIName.toLowerCase()] != null)
    ) {
      return;
    } else {
      await Utility.fillDataType().catch((err) => {
        throw err;
      });
      ...
      let fieldsDetails = await Utility.getFieldsDetails(moduleAPIName).catch(
        (err) => {
          throw err;
        }
      );

      recordFieldDetailsJson = await Initializer.getJSON(
        recordFieldDetailsPath
      );
  ...
  static async verifyModuleAPIName(moduleName) {
    ...
    var recordFieldDetailsPath = await this.getFileName().catch((err) => {
      throw err;
    });
  ...
  static async getFields(moduleAPIName, handlerInstance = null) {
    ...
    await this.getFieldsInfo(this.moduleAPIName, handlerInstance).catch(
      (err) => {
        throw err;
      }
    );
  ...
  static async getFieldsInfo(moduleAPIName, handlerInstance = null) {
    ...
    try {
      if (
        moduleAPIName != null &&
        (await Utility.searchJSONDetails(moduleAPIName).catch((err) => {
          throw err;
        })) != null
      ) {
        return;
      ...
      if (moduleAPIName != null) {
        moduleAPIName = await Utility.verifyModuleAPIName(moduleAPIName).catch(
          (err) => {
            throw err;
          }
        );
        ...
        if (
          ...
          handlerInstance.getModuleAPIName() == null &&
      ...
      recordFieldDetailsPath = await this.getFileName().catch((err) => {
        throw err;
      });

      if (fs.existsSync(recordFieldDetailsPath)) {
        await Utility.fileExistsFlow(
          moduleAPIName,
          recordFieldDetailsPath,
          lastModifiedTime
        ).catch((err) => {
        ...
        this.apiSupportedModule =
          Object.keys(this.apiSupportedModule).length > 0
            ? this.apiSupportedModule
            : await Utility.getModules(null).catch((err) => {
                throw err;
              });

        let recordFieldDetailsJson = fs.existsSync(recordFieldDetailsPath)
      ...
      if (
        ...
        fs.existsSync(recordFieldDetailsPath)
      ) {
        try {
  ...
  static async modifyFields(recordFieldDetailsPath, modifiedTime) {
    let modifiedModules = await this.getModules(modifiedTime).catch((err) => {
      throw err;
    });
    ...
    if (Object.keys(modifiedModules).length > 0) {
      ...
      for (let module in modifiedModules) {
        ...
        await Utility.getFieldsInfo(moduleData[Constants.API_NAME], null).catch(
          (err) => {
            throw err;
          }
        );
  ...
  static async deleteFields(recordFieldDetailsJson, module) {
    ...
    for (let keyName of Object.keys(fieldsJSON)) {
      if (
        ...
        fieldsJSON[keyName][Constants.SUBFORM] == true &&
        recordFieldDetailsJson.hasOwnProperty(
    ...
    if (subformModules.length > 0) {
      for (let subformModule of subformModules) {
        await this.deleteFields(recordFieldDetailsJson, subformModule).catch(
          (err) => {
            throw err;
          }
        );
  ...
  static async getRelatedLists(
    ...
    try {
      ...
      if (
        !fs.existsSync(recordFieldDetailsPath) ||
        (fs.existsSync(recordFieldDetailsPath) &&
          (!Initializer.getJSON(recordFieldDetailsPath).hasOwnProperty(key) ||
            Initializer.getJSON(recordFieldDetailsPath)[key] == null ||
            Initializer.getJSON(recordFieldDetailsPath)[key].length <= 0))
      ) {
        isnewData = true;
        ...
        let relatedListValues = await this.getRelatedListDetails(
          moduleAPIName
        ).catch((err) => {
          throw err;
        });

        recordFieldDetailsJSON = fs.existsSync(recordFieldDetailsPath)
          ? await Initializer.getJSON(recordFieldDetailsPath)
          : {};
      ...
      let moduleRelatedList = recordFieldDetailsJSON.hasOwnProperty(key)
        ? recordFieldDetailsJSON[key]
        : {};

      if (
        !(await this.checkRelatedListExists(
          relatedModuleName,
          moduleRelatedList,
          commonAPIHandler
        ).catch((err) => {
  ...
  static async checkRelatedListExists(
    ...
    for (let index = 0; index < modulerelatedListArray.length; index++) {
      ...
      if (
        relatedListObject[Constants.API_NAME] != null &&
        relatedListObject[Constants.API_NAME].toLowerCase() ==
          relatedModuleName.toLowerCase()
      ) {
        if (
          relatedListObject[Constants.HREF].toLowerCase() ==
        ...
        if (
          relatedListObject[Constants.MODULE].toLowerCase() !=
          Constants.NULL_VALUE
        ) {
  ...
  static async getRelatedListDetails(moduleAPIName) {
    let RelatedListsOperations = (
      await import(
        "../../core/com/zoho/crm/api/related_lists/related_lists_operations.js"
      )
    ).RelatedListsOperations;

    let ResponseWrapper = (
      await import(
        "../../core/com/zoho/crm/api/related_lists/response_wrapper.js"
      )
    ).ResponseWrapper;

    let APIException = (
      await import("../../core/com/zoho/crm/api/related_lists/api_exception.js")
    ).APIException;

    let relatedListArray = [];

    let response = await new RelatedListsOperations(
      moduleAPIName
    ).getRelatedLists();
    ...
    if (response != null) {
      ...
      if (responseObject != null) {
        if (responseObject instanceof ResponseWrapper) {
          ...
          errorResponse[Constants.MESSAGE] = responseObject
            .getMessage()
            .getValue();
      ...
      if (responseHandler != null) {
        if (responseHandler instanceof FieldsResponseWrapper) {
          ...
          if (
            ...
            fieldDetail[Constants.STRUCTURE_NAME] =
              Constants.LINE_TAX_NAMESPACE;
          ...
          if (moduleAPIName.toLowerCase() == Constants.NOTES) {
            ...
            fieldDetail[Constants.STRUCTURE_NAME] =
              Constants.ATTACHMENTS_NAMESPACE;
          ...
          errorResponse[Constants.STATUS] = responseHandler
            .getStatus()
            .getValue();

          errorResponse[Constants.MESSAGE] = responseHandler
            .getMessage()
            .getValue();
  ...
  static async verifyPhotoSupport(moduleAPIName) {
    try {
      ...
      if (
        ...
        modules[moduleAPIName.toLowerCase()] != null
      ) {
  ...
  static async getModuleNames() {
    ...
    var resourcesPath = path.join(
      (await Initializer.getInitializer()).getResourcePath(),
      Constants.FIELD_DETAILS_DIRECTORY
    );
    ...
    let recordFieldDetailsPath = await this.getFileName().catch((err) => {
      throw err;
    });
    ...
    if (
      !fs.existsSync(recordFieldDetailsPath) ||
      (fs.existsSync(recordFieldDetailsPath) &&
        (!fieldMetaJSON.hasOwnProperty(Constants.SDK_MODULE_METADATA) ||
          fieldMetaJSON[Constants.SDK_MODULE_METADATA] == null ||
          fieldMetaJSON[Constants.SDK_MODULE_METADATA].length <= 0))
    ) {
      moduleData = await Utility.getModules(null).catch((err) => {
        throw err;
      });

      await Utility.writeModuleMetaData(
        recordFieldDetailsPath,
        moduleData
      ).catch((err) => {
        throw err;
  ...
  static async writeModuleMetaData(recordFieldDetailsPath, moduleData) {
    let fieldDetailsJSON = fs.existsSync(recordFieldDetailsPath)
      ? Initializer.getJSON(recordFieldDetailsPath)
      : {};
  ...
  static async getModules(header) {
    ...
    let ResponseWrapper = (
      await import("../../core/com/zoho/crm/api/modules/response_wrapper.js")
    ).ResponseWrapper;

    let APIException = (
      await import("../../core/com/zoho/crm/api/modules/api_exception.js")
    ).APIException;

    const ModuleOperations = (
      await import("../../core/com/zoho/crm/api/modules/modules_operations.js")
    ).ModulesOperations;

    const GetModulesHeader = (
      await import("../../core/com/zoho/crm/api/modules/modules_operations.js")
    ).GetModulesHeader;
        ...
        if (responseObject instanceof ResponseWrapper) {
          ...
          modules.forEach((module) => {
            if (module.getAPISupported() == true) {
              ...
              moduleDetails[Constants.GENERATED_TYPE] = module
                .getGeneratedType()
                .getValue();
  ...
  static async refreshModules() {
    ...
    await Utility.getFieldsInfo(null).catch((err) => {
      throw err;
    });
  ...
  static async setDataType(fieldDetail, field, moduleAPIName) {
    ...
    if (
      field.getSystemMandatory() != null &&
      field.getSystemMandatory() == true &&
      !(
    ...
    if (
      keyName.toLowerCase() ==
        Constants.PRICING_DETAILS_API_NAME.toLowerCase() &&
      moduleAPIName.toLowerCase() == Constants.PRICE_BOOKS
    ) {
      ...
      (moduleAPIName.toLowerCase() == Constants.EVENTS ||
        moduleAPIName.toLowerCase() == Constants.ACTIVITIES)
    ) {
      fieldDetail.name = keyName;
      ...
      (moduleAPIName.toLowerCase() == Constants.SOLUTIONS ||
        moduleAPIName.toLowerCase() == Constants.CASES)
    ) {
      fieldDetail.name = keyName;
      ...
      (keyName.toLowerCase() == Constants.TERRITORIES.toLowerCase() ||
        keyName.toLowerCase() == Constants.TERRITORY.toLowerCase()) &&
      field.getCustomField() !== null &&
      field.getCustomField() == false
    ) {
      fieldDetail.name = keyName;
      ...
      keyName.toLowerCase() == Constants.PRODUCT_NAME.toLowerCase() &&
      Constants.INVENTORY_MODULES_ITEMS.includes(moduleAPIName.toLowerCase())
    ) {
      fieldDetail.name = keyName;
      ...
      keyName.toLowerCase() == Constants.DISCOUNT.toLowerCase() &&
      Constants.INVENTORY_MODULES_ITEMS.includes(moduleAPIName.toLowerCase())
    ) {
      fieldDetail.name = keyName;
      ...
      keyName.toLowerCase() == Constants.TAX.toLowerCase() &&
      Constants.PRODUCTS.toLowerCase() == moduleAPIName.toLowerCase()
    ) {
      fieldDetail.name = keyName;
      ...
      if (field.getFormula() != null) {
        ...
        if (
          ...
          Utility.apiTypeVsDataType.get(returnType) != null
        ) {
    ...
    if (apiType.toLowerCase() == Constants.MULTI_SELECT_LOOKUP) {
      ...
      if (
        field.getMultiselectlookup() != null &&
        field.getMultiselectlookup().getLinkingModule() != null
      ) {
    ...
    if (apiType.toLowerCase() == Constants.MULTI_USER_LOOKUP) {
      ...
      if (
        field.getMultiuserlookup() != null &&
        field.getMultiuserlookup().getLinkingModule() != null
      ) {
    ...
    if (apiType == Constants.LOOKUP && field.getLookup() != null) {
      ...
      if (module != null && module.toLowerCase() != Constants.SE_MODULE) {
        ...
        if (
          module.toLowerCase() == Constants.ACCOUNTS &&
          field.getCustomField() != null &&
  ...
  static async fillDataType() {
    ...
    for (let fieldAPIName of fieldAPINameImageUpload) {
      ...
      Utility.apiTypeVsStructureName.set(
        fieldAPIName,
        Constants.IMAGEUPLOAD_NAMESPACE
      );
    }
