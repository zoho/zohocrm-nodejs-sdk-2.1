...
class Utility {
    ...
    static async getFields(moduleAPIName, handlerInstance = null) {
        ...
        try {
            ...
            if(fs.existsSync(recordFieldDetailsPath)) {
                ...
                let apiSupportedmodules = await Utility.getModules(null);
                ...
                if(Object.keys(apiSupportedmodules).length > 0){
                    for (let module in apiSupportedmodules){
                        if(!recordFieldDetailsJson.hasOwnProperty(module)) {
                            let moduleData = apiSupportedmodules.get(module);
    ...
    static async modifyFields(recordFieldDetailsPath, modifiedTime) {
        ...
        if(Object.keys(modifiedModules).length > 0){
            ...
            for(let module of modifiedModules) {
                ...
                await Utility.getFields(moduleData[Constants.API_NAME]);
            }
        }
    }
        ...
        if(response != null) {
            ...
            if(responseObject != null){
                if(responseObject instanceof FieldsResponseWrapper) {
                    let fields = await responseObject.getFields();
    ...
    static async verifyPhotoSupport(moduleAPIName) {
		try {
			...
			if(Constants.PHOTO_SUPPORTED_MODULES.includes(moduleAPIName))
			{
				return true;
			}

            let modules = Utility.getModuleNames();
    ...
    static async setDataType(fieldDetail,field, moduleAPIName) {
        ...
        if(Constants.KEY_VS_INVENTORY_MODULE.has(keyName) && moduleAPIName.toLowerCase() == Constants.KEY_VS_INVENTORY_MODULE.get(keyName).toLowerCase()) {
